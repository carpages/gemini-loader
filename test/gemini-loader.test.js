...
require(['qunit', 'gemini', 'lodash', 'gemini.support'], function (QUnit, G, _, support) {
  QUnit.start();
  ...
  */
  ...
  QUnit.test('Gemini has functionality of jQuery', (assert) => {
    assert.expect(1);

    const gBody = G('body');
    const $body = $('body');

    assert.equal(gBody.get(0), $body.get(0));
  });

  QUnit.test('Gemini has functionality of lodash', function (assert) {
    assert.expect(1);

    const test = [0, 1, null];
    const gTest = G._.compact(test);
    const _test = _.compact(test);

    assert.deepEqual(gTest, _test);
  });

  QUnit.test('Support is loaded', function (assert) {
    assert.expect(1);

    assert.propEqual(G.support, support);
  });
